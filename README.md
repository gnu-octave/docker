...
- [Docker](https://www.docker.com/):
  ...
  docker run -it --rm gnuoctave/octave:8.1.0 octave
  ```
...
- [Singularity](https://sylabs.io/singularity/): most recommended for GUI mode.
  ```sh
  singularity pull docker://gnuoctave/octave:8.1.0

  # Start container (command-line interface)

  singularity run octave_8.1.0.sif
  ```
...
```
singularity exec --bind /run/user octave_8.1.0.sif octave --gui
```
...
docker run \
  ...
  docker.io/gnuoctave/octave:8.1.0 start.sh octave --gui
```
